...
/**
 * Deterministic nonce generation strategy that uses a counter for the
 * invocation field as described in NIST <a
 * href="http://csrc.nist.gov/publications/nistpubs/800-38D/SP-800-38D.pdf">
 * SP-800-38D</a>, section 8.2.1. The invocation part of the sequence is always
 * 64 bits (8 bytes) due to the use of a <code>long</code>, thus the length of
 * the nonce is determined by the length of the fixed part: <code>length = 8 +
 * fixed.length</code>.
 *
 * <p><strong>NOTE:</strong> users of this class are responsible for maintaining
 * the invocation count in order to support enforcement of constraints described
 * in section 8.3; namely the following:</p>
 *
 ...
 * </blockquote>
 *
 * <p>Instances of this class enforce this constraint by considering the nonce
 * length, which determines whether the constraint applies, and the invocation
 * count. The invocation count is incremented upon every invocation of {@link
 * #generate()} method. The current invocation count is accessible via {@link
 * #getInvocations()}.</p>
 *
...
public class CounterNonce implements Nonce
{
  ...
  /**
   * Maximum invocations is 2<sup>32</sup>. Does not apply to nonces with
   * default getLength, {@value #DEFAULT_LENGTH}.
   */
  ...
  /**
   * Creates a new instance. Instances of this method produces nonces of the
   * default length, {@value #DEFAULT_LENGTH}, and are not subject to
   * constraints on the number of invocations.
   *
   * @param  fixed  User-defined fixed field value.
   * @param  invocations  Initial invocation count. The invocations field is
   *                      incremented _before_ use in {@link #generate()}.
